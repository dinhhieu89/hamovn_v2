			...
			switch ( $value['type'] ) {
				...
				case "radio":
					 ...
					 foreach($value['options'] as $option=>$name) {
						$output .= '<label class="radio"><input class="tog" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($smof_data[$value['id']], $option, false) . ' />'.$name.'</label>';				
					}
				...
				case 'multicheck': 			
					...
					foreach ($value['options'] as $key => $option) {
						...
						$output .= '<label class="multicheck" for="'. $of_key_string .'"><input type="checkbox" class="checkbox" name="'.$value['id'].'['.$key.']'.'" id="'. $of_key_string .'" value="1" '. checked($multi_stored[$key], 1, false) .' />'. $option .'</label>';								
					}			 
				break;
				...
				case "skin":
					...
					$i++;
						...
						if(NULL!=checked($select_value, $key, false)) {
							...
							$selected = 'of-radio-skin-selected';  
						}
						
						
						$output .= '<input type="radio" id="of-radio-skin-' . $value['id'] . $i . '" class="checkbox of-radio-img-radio" value="'.$key.'" name="'.$value['id'].'" '.$checked.' />';
						$output .= '<img src="'.get_template_directory_uri().'/admin/assets/images/_blank.gif" alt="'. ucwords($key) .'" class="of-radio-skin-img '. $selected .'" onClick="document.getElementById(\'of-radio-skin-'. $value['id'] . $i.'\').checked = true;" style="background-color: #'.$option['color'].'" data-color="'.$option['color'].'" data-bgcolor="'.$option['bgcolor'].'" data-bgimg="'.$option['bgimg'].'" title="'. ucwords($key) .'" />';
				...
				case 'tiles':
					...
					$i++;
						...
						$filename = preg_replace("/\\.[^.\\s]{3,4}$/", "", basename($option));
						$output .= '<span>';
						...
						$output .= '<div id="tile-'.$filename.'" class="of-radio-tile-img '. $selected .'" style="background: url('.$option.')" onClick="document.getElementById(\'of-radio-tile-'. $value['id'] . $i.'\').checked = true;"></div>';
						$output .= '</span>';				
					}
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ','presslayer').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
