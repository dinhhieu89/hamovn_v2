...
if(class_exists('AQ_Page_Builder')) {
	...
	********************************************************/
	...
	function aq_field_upload($field_id, $block_id, $media, $media_type = 'image') {
		$output = '<input size="40" type="text" id="'. $block_id .'_'.$field_id.'" class="input-full input-upload" value="'.$media.'" name="aq_blocks['.$block_id.']['.$field_id.']">';
	...
	**************************/
	 ...
	 * For dropdown select
	 */
	function get_array_cats(){
		$categories 		= array(); 
		$categories [0] = 'All categories'; 
		$categories_obj 	= get_categories();
		foreach ($categories_obj as $cat) {
			$categories[$cat->cat_ID] = $cat->cat_name . ' ('. $cat->count .')';
		}
		return $categories;
	} 
