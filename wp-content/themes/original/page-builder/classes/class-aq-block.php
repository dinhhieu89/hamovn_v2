	...
	class AQ_Block {
	 	...
	 	function parse_block($block_options) {
	 		$defaults = array(
	 			...
	 			'title' => '',					//title field
	 			'parent' => 0,					//block parent (for blocks inside sections)
	 			'number' => '__i__',			//block consecutive numbering
	 			'first' => false,				//section first
	 		...
	 		echo '<li id="template-block-'.$number.'" class="block block-'.$id_base.' '. $size .' '.$resizable.'">',
	 				'<dl class="block-bar">',
	 					'<dt class="block-handle">',
	 						'<div class="block-title '.$id_base.'">',
	 							$name , $title, 
	 						'</div>',
	 						'<span class="block-controls">',
	 							'<a class="block-edit" id="edit-'.$number.'" title="Edit Block" href="#block-settings-'.$number.'">Edit Block</a>',
								'<a class="block-delete" id="edit-'.$number.'" title="Delete Block" href="#block-settings-'.$number.'">Delete Block</a>',
	 						'</span>',
	 					'</dt>',
	 				'</dl>',
	 				'<div class="show-popup lightbox block-settings clearfix" id="block-settings-'.$number.'">',
					'<div class="lightbox_inner"><a class="media-modal-close popup-close" href="#" title="Close"><span class="media-modal-icon"></span></a><h2>'.$name.'</h2>';
	 	}
	 	...
	 	function after_form($instance) {
	 		...
	 		$block_saving_id = 'aq_blocks[aq_block_'.$number.']';
				...
				echo '<ul class="lightbox_form"><li><div class="title">&nbsp;</div><div class="input">',
					 '<a href="#" class="button button-primary button-large popup-close">Save</a></div></li></ul>';
	 			echo '</div>';
	 	...
	 	function before_block($instance) {
			...
			if($size == 'span-12') $size = $size.' prl-panel';
	 		//echo '<div id="aq-block-'.$template_id.'-'.$number.'" class="aq-block aq-block-'.$id_base.' aq_'.$size.' '.$section_class.' clearfix">';
			echo '<div class="prl-'.$size.' clearfix">';
	 	}
