...
if(!class_exists('AQ_Page_Builder')) {
	class AQ_Page_Builder {
		...
		function admin_enqueue() {
			...
			wp_register_style( 'aqpb-lightbox-css', $this->url.'assets/css/aqpb-lightbox.css', array(), time(), 'all');
			...
			wp_register_script('aqpb-lightbox-js', $this->url . 'assets/js/aqpb-lightbox.js', array('jquery'), time(), true);
			
			// Enqueue 'em
			wp_enqueue_style('aqpb-css');
			wp_enqueue_style('aqpb-blocks-css');
			wp_enqueue_style('aqpb-lightbox-css');
			...
			wp_enqueue_script('aqpb-lightbox-js');
		...
		**/
		function display_template($template_id) {
			...
			if(empty($blocks)) {
				...
				echo '<div id="aq-template-wrapper-'.$template_id.'" class="aq-template-wrapper prl-grid prl-grid-divider prl-panel">';
		...
		function add_media_button( $button ) {
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				if ( version_compare( $wp_version, '3.5', '<' ) ) {
					...
					$img 	= '';
					$output = '<a href="#TB_inline?width=640&inlineId=aqpb-iframe-container" class="add_media thickbox button" title="' . esc_attr__( 'Add Page Template', 'framework' ) . '" style="padding-left: .4em;">' . $img . ' ' . esc_attr__( 'Add Template', 'framework' ) . '</a>';
				}
