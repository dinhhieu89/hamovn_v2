...
jQuery(function($) {
    ...
    var nw_sortable = function(nwWidgetSortable){
        nwWidgetSortable.sortable({
            ...
            stop: function(e,ui) {
                ...
                if( 'prl-accordions' == $id_base || 'prl-tabs' == $id_base ) {
                    ui.item.remove();
                    return;
                }
                if ( ui.item.hasClass('deleting') ) {
                    nwSaveWidget(ui.item);
                    ui.item.remove();
                    return;
                }
                ...
                if ( add ) {
                    ...
                    var widget_id = id_base + '-prl-widget-' + matches;
    ...
    function nwSortableInit(){
        $('#widget-list').children('.widget').draggable( "option", 'connectToSortable', 'div.widgets-sortables,div.prl-widget-extends' );
        nw_sortable( $('#widgets-right .prl-widget-extends') );
    }

    nwSortableInit();

    function nwSaveWidget(widget){
        var container = widget.closest('.prl-widget-extends');
        
        nwSaveWidgetForContainer(container , true );
    }

    function nwSaveWidgetForContainer(container, disabled ){
    ...
    $(document.body).bindFirst('click.widgets-toggle', function(e){
        ...
        if( target.closest('div.widget').find('.prl-widget-extends').length > 0 ) {
            saveWidget = true;
            if ( target.hasClass('widget-control-save') ) {
                var widget = target.closest('div.widget');                                   
                nwSaveWidgetForContainer(widget.find('.prl-widget-extends'));
            } else if ( target.hasClass('widget-control-remove') ) {
                var widget = target.closest('div.widget');
                if( widget.parent().hasClass('prl-widget-extends') ) {
                    var parent = widget.parent();
                    nwSaveWidgetForContainer( parent );
                }
            }
        }
    });
